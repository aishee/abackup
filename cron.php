...
function exitcron()
{
    ...
    if ($config['sendnotification'] && filter_var($config['adminemail'], FILTER_VALIDATE_EMAIL)) {
        ...
        $mail->FromName = 'ABackup';
        $mail->addAddress($config['adminemail']);
        $mail->WordWrap = 50;
        $mail->AddStringAttachment($log,'log.txt');
        $mail->isHTML(true);
        $mail->Subject = 'ABackup backup report';
        $mail->Body    = 'Hello!<br />This is your ABackup backup report at '.date(DATE_RFC822).'.<br/>The backup log is attached.';
        $mail->AltBody = 'Hello! This is your ABackup backup report at '.date(DATE_RFC822).'. The backup log is attached.';
        if(!$mail->send()) {
